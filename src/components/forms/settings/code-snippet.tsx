...
const CodeSnippet = ({ id }: Props) => {
  ...
  // Replace localhost with the correct deployed URL
  const deployedURL = "https://corinna-1.vercel.app"; // Update this with your actual Vercel domain
  ...
  let snippet = `
    ...
    iframe.src = "${deployedURL}/chatbot";
    iframe.classList.add('chat-frame');
    document.body.appendChild(iframe);

    window.addEventListener("message", (e) => {
        if (e.origin !== "${deployedURL}") return;
        let dimensions = JSON.parse(e.data);
        iframe.width = dimensions.width;
        iframe.height = dimensions.height;
        iframe.contentWindow.postMessage("${id}", "${deployedURL}/");
    });
  `;
